...
**Тест от 2026-07-02** — `v9`, сервер `achku-swe`, RAM хоста **3.8 GB**, встреча **~21 мин**, `tracks=3`, `--debug`.

| Метрика | Значение |
|---------|----------|
| **RAM контейнера** (`docker stats`) | пик **345 MiB**, в записи **270–330 MiB**, к концу **~328 MiB** |
| **RAM по `--debug`** (Python + Chromium) | пик **844 MB**, плато **~780–820 MB** |
| **CPU** (`docker stats`) | обычно **10–15%**, пики до **~43%** |
| **Файл opus** | **18.3 MB** (~**0.87 MB/мин**) |
| **Чанки на диск** | ~**80 KB каждые 5 сек** — буфер записи не копится в RAM |

**Вывод:** один бот на VPS **≥1 GB RAM** (`docker run --memory=1g`) — с запасом. Для планирования ориентируйтесь на **`docker stats`** (реальная RAM контейнера); строки `📊 RAM` из `--debug` завышают из‑за суммирования RSS всех процессов Chromium.
...
С v9 чанки сбрасываются на диск каждые 5 сек — RAM под запись **не растёт** всю встречу. По бенчмарку контейнер держит **~300 MiB**; лимит **1 GB** достаточен с запасом.
